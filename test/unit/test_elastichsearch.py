...
class TestZshHistdbIndex(object):
    ...
    def test_mapping_has_dir_fields(self, es):
        mapping = es.indices.get_field_mapping(index='zsh-histdb',
                                               fields='dir')
        field_type = (
            mapping['zsh-histdb']['mappings']['dir']['mapping']['dir']['type'])
        assert field_type == 'keyword'
    ...
    def test_mapping_has_status_fields(self, es):
        mapping = es.indices.get_field_mapping(index='zsh-histdb',
                                               fields='status')
        field_type = (mapping['zsh-histdb']['mappings']['status']['mapping']
                      ['status']['type'])
        assert field_type == 'short'

    def test_mapping_has_session_fields(self, es):
        mapping = es.indices.get_field_mapping(index='zsh-histdb',
                                               fields='session')
        field_type = (mapping['zsh-histdb']['mappings']['session']['mapping']
                      ['session']['type'])
        assert field_type == 'integer'

    def test_mapping_has_duration_fields(self, es):
        mapping = es.indices.get_field_mapping(index='zsh-histdb',
                                               fields='duration')
        field_type = (mapping['zsh-histdb']['mappings']['duration']['mapping']
                      ['duration']['type'])
        assert field_type == 'integer'
